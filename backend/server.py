...
def _send_email_notification(form: ContactForm):
    """Send email notification when someone submits the contact form"""
    try:
        smtp_email = os.environ.get('SMTP_EMAIL')
        smtp_password = os.environ.get('SMTP_PASSWORD')
        notify_email = os.environ.get('NOTIFY_EMAIL', 'niteshsingh15151@gmail.com')
...
Name: {form.name}
Email: {form.email}
Time: {form.timestamp.strftime('%Y-%m-%d %H:%M:%S')} UTC
...
"""
        ...
        logger.info(f"Email notification sent for contact from {form.name}")
    except Exception as e:
